...
namespace ButteryFixes.Patches.General
{
    ...
    class StormyWeatherPatches
    {
        ...
        static IEnumerable<CodeInstruction> StormyWeatherTransUpdate(IEnumerable<CodeInstruction> instructions)
        {
            ...
            {
                if (insertAt < 0 && codes[i].opcode == OpCodes.Brtrue && codes[i - 1].opcode == OpCodes.Ldfld && (FieldInfo)codes[i - 1].operand == isInFactory && codes[i - 4].opcode == OpCodes.Ldfld && (FieldInfo)codes[i - 4].operand == metalObjects)
                    insertAt = i - 5;
                else if (codes[i].opcode == OpCodes.Ldfld && (FieldInfo)codes[i].operand == setStaticGrabbableObject)
                {
                    if (codes[i - 6].opcode == OpCodes.Ldfld && (FieldInfo)codes[i - 6].operand == metalObjects)
                    {
                        codes[i].operand = metalObjects;
                        codes.InsertRange(i + 1, [
                            new CodeInstruction(codes[i - 5].opcode, codes[i - 5].operand),
            ...
            if (insertAt >= 0)
            {
                codes.InsertRange(insertAt, [
                    ...
                    new CodeInstruction(codes[insertAt + 2].opcode, codes[insertAt + 2].operand),
